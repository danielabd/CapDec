...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args, warmup_steps: int = 5000, output_dir: str = ".", output_prefix: str = ""):
    ...
    for epoch in range(epochs):
        ...
        if args.val_pt:
            ...
            if val_loss / len(val_dataloader)< best_val_los:
                best_val_los = val_loss / len(val_dataloader)
                best_epoch_val_los = epoch
            wandb.log({'train/loss': accumulated_loss / len(train_dataloader),
                       'val/loss': val_loss / len(val_dataloader),
                       'val/best_val_los': best_val_los,
                       'val/best_epoch_val_los': best_epoch_val_los})
        print(f"best val los = {best_val_los} for epoch = {best_epoch_val_los}")
...
def main():
    ...
    parser.add_argument('--out_dir', default=os.path.join(os.path.expanduser('~'),'checkpoints','coco_train'), help='path to output directory')
